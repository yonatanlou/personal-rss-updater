...
class BlogStorage:
    ...
    def __init__(self, storage_path: Optional[Path] = None):
        ...
        self.storage_path = storage_path or Path("data/blog_states.json")
    ...
    def increment_failure_count(self, blog_name: str, url: str = "") -> None:
        ...
        self.update_blog_state(
            blog_name, url=url, failure_count=failure_count, last_check=datetime.now()
        )
    ...
    def get_failed_blogs(self, threshold: int = 3) -> Dict[str, BlogState]:
        ...
        return {
            name: state
            for name, state in self.blog_states.items()
    ...
    def get_summary(self) -> Dict:
        ...
        last_checks = [state.last_check for state in self.blog_states.values() if state.last_check]
    ...
    def sync_with_blogs_config(self, blogs_config_path: Optional[Path] = None) -> Dict:
        ...
        """
        try:
            result = self.sync_manager.sync_with_blogs_config(self.blog_states, blogs_config_path)
    ...
    def get_blogs_needing_biweekly_reminder(self) -> Dict[str, BlogState]:
        ...
        for blog_name, state in self.blog_states.items():
            ...
            needs_reminder = state.is_problematic or state.failure_count >= 5
    ...
    def remove_blog_state(self, blog_name: str) -> bool:
        """
        Remove a blog state from storage.

        Args:
            blog_name: Name of the blog to remove

        Returns:
            True if blog was found and removed, False otherwise
        """
        if blog_name in self.blog_states:
            del self.blog_states[blog_name]
            return True
        return False
